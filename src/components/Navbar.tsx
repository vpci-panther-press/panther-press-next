...
function HeaderLink({ href, children, className, target }: { href: string; children: React.ReactNode; className?: string; target?: string }) {
	return (
		<Link
			href={href}
			target={target}
...
export function Navbar() {
	...
	return (
		<header className="z-50 w-full rounded-lg bg-zinc-200/90 py-4 font-manrope backdrop-blur sm:flex-nowrap sm:justify-start md:bg-transparent dark:bg-zinc-900/80 md:dark:bg-transparent">
			<nav className="mx-auto w-full max-w-[85rem] px-4 sm:flex sm:items-center sm:justify-between" aria-label="Global">
				<div className="flex items-center justify-between">
					<Link className="flex-none text-2xl font-semibold text-zinc-900 dark:text-white" href="/">
					...
					<div className="mt-4 sm:ml-auto ml-4 flex flex-col gap-5 text-lg font-semibold sm:mt-0 sm:flex-row sm:items-center sm:justify-end sm:pl-5">
						...
						<div className="flex flex-row gap-2 align-middle sm:ml-2 basis-full">
							{SOCIALNETWORKS.map((network) => {
								...
								return (
									<HeaderLink key={network.name} className="my-auto" href={network.url} target="_blank" aria-label={network.name}>
										<Icon className="size-5" />
									</HeaderLink>
								)
							})}
							<ThemeToggle />
							{isLargeScreen && <Suspense fallback={<div className="w-32 sm:w-44" />}><Search /></Suspense>}
						</div>
						{!isLargeScreen && <Suspense fallback={<div className="w-32" />}><Search /></Suspense>}
					</div>
				</div>
			</nav>
		</header>
