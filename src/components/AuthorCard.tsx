	...
	return (
		...
		>
			...
			<div className="relative z-10 p-3 text-left">
				<p className="text-xs font-semibold uppercase tracking-wide text-cs-secondary">{author.board?.join(" & ")}</p>
				<h3 className="text-lg font-bold leading-tight">{author.name}</h3>
				<p className="text-sm text-gray-200">{author.role}</p>
				{author.alumni && <span className="mt-1 inline-block rounded-full bg-white/20 px-2 py-0.5 text-xs font-semibold">Alumni</span>}
			</div>
		</Link>
	)
}
