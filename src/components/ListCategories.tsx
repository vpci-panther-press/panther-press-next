...
export function ListCategories({ activeCategory, currentPath }: { activeCategory?: string; currentPath?: string }) {
	// Consider "View All" active when path points to /articles (with optional ?page)
	const isAll = currentPath ? (currentPath.startsWith("/articles") && !currentPath.includes("/category/")) : !activeCategory
	...
	const activeClasses = "border-black text-black dark:border-white dark:text-white"

	return (
		<div className="flex flex-col">
			<div className="flex max-w-full min-w-full gap-5">
				<Link href="/articles?page=1" className={cn(baseClasses, isAll && activeClasses)}>
					View All
				</Link>
				{CATEGORIES.map((category) => {
					...
					return (
						<Link
							...
							className={cn(baseClasses, activeCategory?.toLowerCase() === slug && activeClasses)}
						>
							{category.trim()}
						</Link>
					)
