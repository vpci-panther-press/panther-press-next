...
function getPaginationItems(
  ...
  if (end < (endPages.length ? endPages[0] : totalPages) - 1)
    items.push("dots");
  ...
  return (
    ...
    >
      ...
      {currentPage > 1 ? (
        ...
        <span
          aria-disabled="true"
          className={`${baseItemClasses} disabled:opacity-50 text-zinc-500 dark:text-zinc-400`}
        >
          <svg
            className="h-3.5 w-3.5 shrink-0"
            xmlns="http://www.w3.org/2000/svg"
        ...
        {items.map((item, idx) => {
          if (item === "dots") {
            return (
              <li
                key={`dots-${idx}`}
                className={`${baseItemClasses} text-zinc-500 dark:text-zinc-400`}
                aria-hidden
              >
                …
              </li>
