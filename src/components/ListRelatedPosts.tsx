...
export function ListRelatedPosts({ posts }: { posts: Post[] }) {
	if (!posts.length) return <span className="text-gray-500">There are no related posts yet. 😢</span>
	return (
		<section className={cn(`grid lg:grid-cols-3 lg:grid-rows-1 grid-cols-1 grid-rows-3 gap-8`)}>
			{posts.map((post) => (
				<Link
					key={post.id}
				...
				>
					...
					<div className="my-6">
						...
						<p className="mt-5 text-gray-600 dark:text-gray-400">{post.description}</p>
					</div>
					<div className="mt-auto flex items-center gap-x-3">
						<div>
							<p className="text-sm text-gray-800 dark:text-gray-200">{post.readTime}</p>
						</div>
					</div>
				</Link>
			))}
