  ...
  return (
    <article className="min-w-full sm:max-w-none md:max-w-none md:py-4">
      {isEnabled && post.draft && (
        <div
          className="bg-red-50 border-s-4 border-red-500 p-4 dark:bg-red-800/30 my-3"
          role="alert"
          ...
          <div className="flex">
            <div className="ms-3">
              ...
              <p className="text-sm text-gray-700 dark:text-neutral-400">
                Draft mode is currently enabled. You are viewing a draft version
                of this article. Only you can see this.
              </p>
            </div>
          </div>
        </div>
      ...
      <header className="mb-3 flex flex-col items-center justify-center gap-6">
        <div className="flex w-full flex-col gap-2 text-center font-semibold">
          <Link
            href={`/issues/${post.issue.slug}`}
            className="text-cs-primary underline sm:no-underline sm:hover:underline dark:text-cs-secondary"
          >
            {post.issue.name}
          </Link>
          <h1 className="text-center text-5xl font-semibold text-balance md:mb-2.5 md:text-6xl">
            {post.title}
          </h1>
          <p>{post.description}</p>
          <div className="mt-4 flex flex-col flex-wrap items-center justify-center text-base sm:flex-row">
            <div className="border-gray-300 sm:border-r sm:pr-2 dark:border-gray-700">
              {post.category ? <Tag tag={post.category} category /> : null}
            </div>
        ...
        <div className="md:max-w-prose lg:max-w-full">
          ...
          <footer>
            <div className="mb-10 flex w-full flex-col items-center justify-center gap-10 md:hidden">
              ...
              />
              <AuthorProfile
                authors={post.authors as Post_Author[]}
                imageCredit={post.photoCredits}
              />
            </div>
...
async function getRelatedPosts(slug: string) {
  ...
  const getAuthorId = (author: Post_Author) => {
    ...
    return typeof author.author_id === "object"
      ? (author.author_id as Author).id
  ...
  const related = await Promise.all(
    posts
      ...
      .map(async (p) => {
        const currentAuthorIds = current.authors
          .map(getAuthorId)
          .filter(Boolean);
        ...
        const isRelated =
          ...
          (current.issue.name === p.issue.name &&
  ...
  return related
    .filter((p): p is NonNullable<(typeof related)[number]> => Boolean(p))
    .slice(0, MAX_POSTS);
}
