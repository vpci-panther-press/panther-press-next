...
export default async function CategoryPage({ params }: { params: Promise<{ category: string; page: string }> }) {
	...
	const matchedCategory = CATEGORIES.find((cat) => sluglify(cat.toLowerCase()) === category)
	if (!matchedCategory) {
		notFound()
	}
		...
		<div className="mb-10 space-y-6">
			...
			<div>
				<div className="flex flex-row justify-between">
					...
					<h2 className="text-end text-lg font-medium tracking-wide capitalize">{matchedCategory}</h2>
				</div>
				<ListPosts FirstBig posts={posts} />
			</div>
			<Pagination currentPage={pageNumber} lastPage={totalPages} makeHref={(page) => `/articles/category/${sluglify(normalized)}/${page}/`} />
		</div>
	)
}
